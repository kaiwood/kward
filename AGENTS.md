...
- User-entered external `/shell` commands and `!command` use interactive PTY terminal handoff by default. Safe, line-oriented output from commands that did not read keyboard input may be mirrored into the transient transcript; keep full-screen and keyboard-driven output terminal-owned. `capture <command>` retains bounded, sanitized shell output, while `/capture <command>` provides bounded one-shot transcript output. Keep model-requested and RPC `run_shell_command` workers captured, cancellable, sandbox-aware, and noninteractive.
