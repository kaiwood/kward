...
- User-entered external `/shell` commands and `!command` use interactive PTY terminal handoff by default; `capture <command>` and `/capture <command>` retain bounded, sanitized transcript output. Keep model-requested and RPC `run_shell_command` workers captured, cancellable, sandbox-aware, and noninteractive.
