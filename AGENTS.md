...
- User-entered external `/shell` commands and `!command` use adaptive interactive PTY handoff by default: conservative line/progress controls render above a frozen composer, while screen-oriented or unknown controls trigger permanent full-terminal passthrough for that command. Keep the output classifier byte-preserving and incremental across chunks, suppress Kward rendering throughout child ownership, and retain at most bounded transcript-safe output captured before the first forwarded input byte. `capture <command>` retains bounded, sanitized shell output, while `/capture <command>` provides bounded one-shot transcript output. Keep model-requested and RPC `run_shell_command` workers captured, cancellable, sandbox-aware, and noninteractive.
