  ...
  module ConfigFiles
    ...
    def skills_registry
      Skills::Registry.new(
        ...
        markdown_parser: method(:skill_markdown_parts),
    ...
    def markdown_parts(path)
      ...
      data = yaml_text.to_s.empty? ? {} : safe_load_frontmatter(yaml_text)
      frontmatter = data.is_a?(Hash) ? data.transform_keys(&:to_s) : {}
      [frontmatter, body]
    end

    def skill_markdown_parts(path)
      markdown_parts(path)
      ...
      [lenient_frontmatter(yaml_text), body]
    end

    def safe_load_frontmatter(yaml_text)
