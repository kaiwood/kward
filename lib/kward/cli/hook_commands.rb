    ...
    module HookCommands
      ...
      def handle_hooks_command(argument)
        ...
        when "doctor"
          ...
          runtime_output("Usage: /hooks [list|events|logs|doctor]")
        end
      end

      def print_hooks_list
        lines = ["Lifecycle hooks"]
        entries = configured_hook_entries + plugin_hook_entries
        if entries.empty?
      ...
      def print_hooks_doctor
        ...
        begin
          ...
          lines << "Config hooks: #{manager.handlers.length}"
      ...
      def configured_hook_entries
        hooks = ConfigFiles.read_config["hooks"]
