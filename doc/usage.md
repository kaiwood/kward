...
In chat mode, the agent shows a boxed bottom composer. It can inspect the workspace with `list_directory` and `read_file`, safely write files with `write_file`, edit existing files with `edit_file`, run shell commands with `run_shell_command` after confirmation, search the web with `web_research`, and ask structured clarification questions with `ask_user_question`.

Existing files must be read in the current conversation before writing or editing, and every write asks for confirmation first.

## Sessions

Interactive chats are saved as per-workspace JSONL sessions under `~/.kward/sessions/`. Type `/exit` or `/quit` to leave.

Use `/new` to start fresh, `/resume` to pick a saved session, `/name <name>` to name the current session, `/clone` to duplicate it, `/export [path]` to write a Markdown transcript, and `/redraw` to refresh the visible terminal after resize glitches.
