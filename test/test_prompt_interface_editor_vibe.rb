...
class TestPromptInterfaceEditorVibe < KwardTestCase
  ...
  def test_prompt_interface_vibe_mode_gj_joins_without_space
    Dir.mktmpdir do |dir|
      ...
      Dir.chdir(dir) do
        ...
        "gJ".each_char { |key| prompt.send(:handle_editor_key, key) }

        editor = prompt.instance_variable_get(:@editor_state)
        assert_equal "onetwo", editor.buffer
      end
    end
  end
  ...
  def test_prompt_interface_vibe_mode_ge_motions_move_to_previous_word_end
    Dir.mktmpdir do |dir|
      ...
      Dir.chdir(dir) do
        ...
        assert prompt.send(:open_editor, "notes.txt")
        editor = prompt.instance_variable_get(:@editor_state)
        editor.set_cursor_line_and_column(0, 15)

        "ge".each_char { |key| prompt.send(:handle_editor_key, key) }
  ...
  def test_prompt_interface_vibe_mode_pipe_moves_to_counted_column
    Dir.mktmpdir do |dir|
      ...
      Dir.chdir(dir) do
        ...
        "4|".each_char { |key| prompt.send(:handle_editor_key, key) }
  ...
  def test_prompt_interface_vibe_mode_g_moves_to_last_non_blank_character
    Dir.mktmpdir do |dir|
      File.write(File.join(dir, "notes.txt"), "alpha   ")
      Dir.chdir(dir) do
        prompt = Kward::PromptInterface.new(input: StringIO.new, output: StringIO.new, editor_mode: "vibe")
