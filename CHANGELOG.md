...
- Changed large tool results to be compacted before they enter model context while preserving full originals in session tool-execution records, avoiding storage for verbatim outputs, preserving short errors exactly, and reverting automatically when compaction would not reduce context.
